...
async def receive_device_heartbeat(
    ...
    return WebhookResponse(
        status="accepted",
        event_id=f"heartbeat_{device_id}_{int(_utcnow().timestamp())}",
...
async def update_twin_graph(device_id: str, sensor_data: Dict[str, Any]) -> None:
    ...
    twin_graph_data[device_id]["last_seen"] = _utcnow().isoformat()
...
async def update_device_heartbeat(device_id: str) -> None:
    ...
    twin_graph_data[device_id]["last_heartbeat"] = _utcnow().isoformat()
