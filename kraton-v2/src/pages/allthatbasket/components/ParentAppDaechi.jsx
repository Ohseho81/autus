...
export default function ParentAppDaechi({ onBack }) {
  ...
  return (
    <div className="min-h-screen" style={{ background: colors.background }}>
      {/* Header */}
      <div className="px-5 pt-12 pb-5" style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
          ...
          {mockChildren.map(child => (
            <motion.button
              ...
              className={'flex items-center gap-3 px-4 py-3 rounded-2xl min-w-[180px] transition-all ' +
                (child.id === selectedChild ? 'bg-white/30 shadow-lg' : 'bg-white/15 hover:bg-white/20')
              }
            >
      ...
      <div className="flex bg-white border-b border-gray-200 sticky top-0 z-10">
        {tabs.map(tab => (
          ...
          >
            ...
            {activeTab === tab.id && (
              <motion.div layoutId="tabIndicator" className="absolute bottom-0 left-0 right-0 h-0.5 bg-indigo-600" />
            )}
          </button>
        ))}
      </div>
        ...
        <AnimatePresence mode="wait">
                  ...
                  {['월', '화', '수', '목', '금', '토', '일'].map((d, i) => (
                    <div key={d} className="text-center">
                      ...
                      <div className={'w-8 h-8 rounded-full flex items-center justify-center text-sm ' +
                        (i === 3 ? 'bg-gradient-to-br from-indigo-500 to-purple-600 text-white font-medium' : 'text-gray-600')
                      }>
          ...
          {activeTab === 'payment' && (
            <motion.div key="payment" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
              <div className="rounded-2xl p-5 flex items-center justify-between" style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
              ...
              <div className="bg-white rounded-2xl p-4 shadow-sm">
                ...
                <div className="space-y-3">
                  ...
                  <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: (mockPayment.lessonsRemaining / mockPayment.lessonsTotal) * 100 + '%', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }} />
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          {activeTab === 'videos' && (
            <motion.div key="videos" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
              {unwatchedVideos > 0 && (
                <div className="rounded-xl p-4 flex items-center justify-between" style={{ background: 'linear-gradient(135deg, #10B981 0%, #059669 100%)' }}>
              ...
              <div className="space-y-3">
                {mockVideos.map(video => (
                  <motion.div key={video.id} whileHover={{ scale: 1.01 }} className="bg-white rounded-xl p-3 shadow-sm flex gap-3 relative">
                    {!video.viewed && (
                    ...
                    <div className="w-24 h-18 rounded-lg bg-gray-200 flex items-center justify-center relative">
                      ...
                      <span className="absolute bottom-1 right-1 bg-black/70 text-white text-xs px-1.5 rounded">{video.duration}</span>
                    </div>
                    <div className="flex-1">
                      ...
                      <div className="flex gap-1.5 mt-2 flex-wrap">
                        {video.skillTags.map((tag, i) => (
                          <span key={i} className="px-2 py-0.5 bg-indigo-50 text-indigo-600 text-xs rounded-full">{tag}</span>
                        ))}
                      </div>
                    </div>
                  </motion.div>
          ...
          {activeTab === 'chat' && (
            <motion.div key="chat" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              className="bg-white rounded-2xl overflow-hidden shadow-sm" style={{ height: 'calc(100vh - 300px)' }}>
              <div className="h-full flex flex-col">
                  ...
                  {chatMessages.map(msg => (
                    <div key={msg.id} className={'mb-4 ' + (msg.type === 'user' ? 'text-right' : '')}>
                      <div className={'inline-block max-w-[85%] px-4 py-3 rounded-2xl ' +
                        (msg.type === 'bot' ? 'bg-gray-100 text-gray-800 rounded-bl-sm' : 'bg-indigo-600 text-white rounded-br-sm')
                      }>
                        {msg.content.split('\n').map((line, i) => (
                ...
                <div className="p-4 border-t border-gray-200 flex gap-2">
                  <input type="text" placeholder="메시지를 입력하세요..." value={inputValue}
