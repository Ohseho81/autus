-- ============================================================================
-- message_outbox + message_log (온리쌤 메시징 모듈)
-- vercel-api lib/messaging/outbound-worker.ts 의존
-- ============================================================================
...
-- message_log: 발송 이력 (outbound-worker에서 DELIVERED 등 기록)
...
-- RLS: service_role만 write, anon/authenticated 읽기 (선택)
...
CREATE POLICY "message_outbox service write" ON message_outbox
  FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "message_log service write" ON message_log;
CREATE POLICY "message_log service write" ON message_log
  FOR ALL USING (true) WITH CHECK (true);

COMMENT ON TABLE message_outbox IS '온리쌤 메시징 발송 큐. Kakao Worker가 처리.';
COMMENT ON TABLE message_log IS '메시지 발송 이벤트 로그.';
