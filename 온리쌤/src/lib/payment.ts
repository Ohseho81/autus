...
import { WEB_URLS, getSupabaseFunctionUrl } from '../config/api-endpoints';

// PortOne V2 설정
const PORTONE_CONFIG = {
  storeId: env.payment.portone.storeId,
  channelKey: env.payment.portone.channelKey,
  // 카카오페이 채널
  kakaoPayChannelKey: env.payment.portone.kakaoPayChannel,
};
...
export function generatePaymentUrl(request: PaymentRequest): string {
  ...
  const params = new URLSearchParams({
    ...
    // 리다이렉트 URL
    redirectUrl: `atb://payment/complete?orderId=${orderId}`,
    // Webhook URL (Supabase Edge Function)
    noticeUrls: JSON.stringify([
      getSupabaseFunctionUrl('payment-webhook'),
    ]),
  });

  return `${WEB_URLS.portone.checkout}?${params.toString()}`;
}
