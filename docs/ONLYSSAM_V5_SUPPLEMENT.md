...
|------|------|
| **저장** | 리포트 본문 → Notion 저장 |
| **발송** | 알림톡에는 Notion 링크만 발송 |
| **효과** | 비용 절감, 가독성 향상, 추후 열람 용이 |

→ 월간 리포트 템플릿: "OOO님의 2월 성장 리포트가 준비되었습니다. [보러가기]"
...
1. **리포트 Notion 연동** — 리포트 본문 Notion 저장 → 알림톡에는 링크만 발송 (비용·가독성)
2. **Dead-letter 알림** — 3회 실패 시 몰트봇 또는 이메일 알림
3. **message_log 감사 확장** — actor, reason 필드 추가
4. **Safety Chain 로직 정리** — "참석+실제 불참" 대조 명확화, safety_alerts/safety_log 역할 정리
5. **마이그레이션 파일 정리** — DB 스키마와 코드/문서 일치
6. **카카오 비즈니스 채널 + 카카오싱크 앱** 등록 및 연동
7. **사전출결 알림톡 템플릿** — [참석]/[결석] 버튼 템플릿 승인
