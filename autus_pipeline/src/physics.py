...
class PhysicsNode:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "name": self.name,
            "direct_money": self.direct_money_krw,
            "time_cost": self.time_cost_krw,
            "synergy_money": self.synergy_money_krw,
            "total_value": self.total_value_krw,
...
class PhysicsResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "direct_money": self.total_direct_money,
            "time_cost": self.total_time_cost,
            "synergy_money": self.total_synergy_money,
            "future_value_12m": self.future_value_12m,
            "growth_rate": self.growth_rate,
            "delete_targets": self.delete_targets,
...
def from_pipeline_result(
    ...
    for role in roles:
        ...
        role_name = role.get("role", role.get("primary_role", "UNKNOWN"))
        ...
        # 시간 계산 (분 → 시간)
        minutes = role.get("effective_minutes", kpi.get("effective_minutes", 0) / max(1, len(roles)))
        hours = minutes / 60 if minutes else 40  # 기본 40시간
    ...
    # 시너지 연결 생성
    for syn in synergy:
        # pair 형식 처리 (i_j 또는 pair 필드)
        pair = syn.get("pair", "")
        if not pair and "i" in syn and "j" in syn:
            pair = f"{syn['i']}_{syn['j']}"
        
        if "_" in str(pair):
            parts = str(pair).split("_", 1)
            if len(parts) == 2:
                a, b = parts
                uplift = syn.get("uplift", syn.get("pair_uplift", syn.get("synergy_uplift_per_min", 0)))
                
                # 시너지 강도 계산 (uplift 기반)
                strength = min(1.0, max(0.1, abs(uplift) / 100)) if uplift else 0.1
...
def analyze_physics(
    kpi: Dict[str, Any],
    roles_df,  # pandas DataFrame
    synergy_df,  # pandas DataFrame
    out_dir: str = "data/output"
) -> PhysicsResult:
    """
    Physics Map 분석 실행 및 저장
    """
    import os
    
    # DataFrame → Dict 변환
    roles = roles_df.to_dict('records') if hasattr(roles_df, 'to_dict') else []
    synergy = synergy_df.to_dict('records') if hasattr(synergy_df, 'to_dict') else []
    
    # 엔진 생성 및 계산
    engine = from_pipeline_result(kpi, roles, synergy)
    result = engine.compute()
    
    # JSON 저장
    os.makedirs(out_dir, exist_ok=True)
    
    json_path = os.path.join(out_dir, "physics_map.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(result.to_dict(), f, ensure_ascii=False, indent=2, default=str)
    
    # 리포트 저장
    report_path = os.path.join(out_dir, "physics_report.txt")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(engine.report())
