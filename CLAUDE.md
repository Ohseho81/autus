...
- **DB**: Supabase `pphzvnaedmzcvpxjulti` (~140 tables, 37 Edge Functions, 136 migrations)
- **Current Focus**: 온리쌤 안정화 (Phase 1) → 뷰티 스펙 정리
...
> 상세 지시서: **ONLYSSAM.md** 참조
> TASK-1(RLS보안) → TASK-2(중복org) → TASK-3(출결) → TASK-4(카카오) → TASK-5(search_path) → TASK-6(모니터링)
